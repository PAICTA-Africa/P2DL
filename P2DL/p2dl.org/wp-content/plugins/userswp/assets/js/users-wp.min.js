...
function uwp_modal_loading(inputs) {
    ...
    var $modal_content = '<div class="modal-header">' + '<span class="badge badge-pill badge-light p-0 mt-2 w-25 bg-loading">&nbsp;</span></div>' + '<div class="modal-body text-center">' + $inputs + "</div>";
    var $modal = '<div class="modal fade uwp-auth-modal bsui" tabindex="-1" role="dialog" aria-labelledby="uwp-profile-modal-title" aria-hidden="true">' + '<div class="modal-dialog modal-dialog-centered">' + '<div class="modal-content">' + $modal_content + "</div></div></div>";
...
function uwp_maybe_check_recaptcha($form) {
    if (typeof uwp_recaptcha_loops === "undefined" || !uwp_recaptcha_loops) {
        uwp_recaptcha_loops = 1
    }
...
function uwp_modal_login_form_process() {
    ...
    jQuery.ajax({
        ...
        data: data,
        beforeSend: function() {
            jQuery(".uwp-auth-modal .modal-content .uwp_login_submit").html('<i class="fas fa-circle-notch fa-spin"></i> ' + $button_text).prop("disabled", true);
            jQuery(".uwp-auth-modal .modal-content .modal-error").html("");
            return uwp_maybe_check_recaptcha("login")
        },
        success: function(data) {
            if (data.success == true) {
                jQuery(".uwp-auth-modal .modal-content .uwp_login_submit").html($button_text).prop("disabled", true);
                ...
                jQuery(".uwp-auth-modal .modal-content .modal-error").html(data.data.message);
                jQuery(".uwp-auth-modal .modal-content .uwp_login_submit").html($button_text).prop("disabled", false);
...
function uwp_modal_login_form_2fa_process(type, fields) {
    ...
    jQuery.ajax({
        ...
        data: data,
        beforeSend: function() {
            $button.html('<i class="fas fa-circle-notch fa-spin"></i> ' + $button_text).prop("disabled", true);
            jQuery(".uwp-auth-modal .modal-content .modal-error").html("")
        },
        success: function(data) {
            if (data.success == true) {
                $button.html($button_text).prop("disabled", true);
                jQuery(".uwp-auth-modal .modal-content .modal-error").html(data.data.message);
                if (data.data.redirect) {
                    setTimeout(function() {
                        location.href = data.data.redirect
...
function uwp_modal_forgot_password_form() {
    var data = {
        action: "uwp_ajax_forgot_password_form"
    };
    jQuery.ajax({
        type: "POST",
        url: uwp_localize_data.ajaxurl,
...
function uwp_checkPasswordStrength($pass1, $pass2, $strengthResult, $submitButton, blacklistArray) {
    var pass1 = $pass1.val();
    var pass2 = $pass2.val();
    ...
    if (confirm(message)) {
        jQuery.ajax({
            ...
            success: function(data) {
                ...
                var $modal = '<div class="modal fade uwp-gd-modal bsui" tabindex="-1" role="dialog" aria-labelledby="uwp-gd-modal-title" aria-hidden="true">' + '<div class="modal-dialog modal-dialog-centered">' + '<div class="modal-content">' + $modal_content + "</div></div></div>";
